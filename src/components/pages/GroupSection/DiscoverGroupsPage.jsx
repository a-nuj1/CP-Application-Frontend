...
const mockGroups = [
  {
    id: 1,
    name: "DSA Champions",
    description: "For competitive programmers preparing for interviews and contests",
    members: 245,
...
export default function DiscoverGroupsPage() {
  ...
  const categories = ['All', 'Competitive Programming', 'Interview Prep', 'Web Development', 'Algorithms', 'Data Structures'];
      ...
      />
        ...
        <div className="text-center mb-12">
          ...
          <p className="max-w-xl mx-auto text-xl text-gray-300">
            Join communities of like-minded coders. Learn together, compete together, grow together.
          </p>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="flex items-center">
                    ...
                    />
                    <span className="ml-3 text-yellow-400 font-medium">{minRating.toFixed(1)}</span>
                  </div>
                </div>
                
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Members Range</label>
                  <div className="flex items-center space-x-4">
                    <input
                      ...
                      max="1000"
                      value={memberRange[0]}
                      onChange={(e) => setMemberRange([parseInt(e.target.value), memberRange[1]])}
                    ...
                    <input
                      ...
                      max="1000"
                      value={memberRange[1]}
                      onChange={(e) => setMemberRange([memberRange[0], parseInt(e.target.value)])}
                      className="w-20 px-3 py-2 border border-gray-700 rounded-md bg-gray-700 text-gray-200 focus:outline-none focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm"
                    />
                  </div>
                </div>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {categories.map(category => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-colors duration-200 ${selectedCategory === category ? 'bg-yellow-500 text-gray-900' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
              >
                {category}
              </button>
            ))}
        ...
        {filteredGroups.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredGroups.map(group => (
              <div key={group.id} className="bg-gray-800 rounded-lg shadow-lg overflow-hidden hover:shadow-xl border border-gray-700 hover:border-yellow-400 transition-all duration-300">
                <div className="p-6">
                  ...
                  <div className="mt-3 flex items-center">
                    <div className="flex items-center">
                      {[...Array(5)].map((_, i) => (
                        <FaStar 
                          key={i} 
                          className={`h-4 w-4 ${i < Math.floor(group.rating) ? 'text-yellow-400' : 'text-gray-600'}`}
                        />
                      ))}
                      <span className="ml-1 text-sm text-yellow-400">{group.rating}</span>
                    </div>
                  </div>
          ...
          <div className="text-center py-12 bg-gray-800 rounded-lg shadow-lg border border-gray-700">
            ...
            <h3 className="mt-2 text-lg font-medium text-gray-100">No groups found</h3>
